...
const appearance = matter(fs.readFileSync(path.join(source, 'content/settings/appearance/speciality-themes.md'), 'utf8')).data
...
copy('content/settings/appearance')
copy('templates/doctor-starter/en', 'content/pages/en')
for (const entry of ['experience.md', 'awards.md', 'memberships.md', 'publications.md', 'services']) {
  const target = path.join(destination, 'content/doctor/en', entry)
  fs.mkdirSync(path.dirname(target), { recursive: true })
  fs.renameSync(path.join(destination, 'content/pages/en', entry), target)
}
write('content/settings/site.md', {
...
write('content/doctor/en/profile.md', {
...
write('content/doctor/photo.md', { profileImage: '/doctor-placeholder.svg' }, '\nUpload your own portrait under public/ and update this path.\n')
write('content/doctor/contact.md', { phone: '', email: '', whatsapp: '', bookingPhone: '', bookingUrl: '', latitude: null, longitude: null }, '\nKeep phone numbers in quotes. Leave bookingPhone empty to reuse phone. Use a full HTTPS URL for online booking.\n')
write('content/doctor/en/chamber.md', { chamber: { name: '', address: '', visitingDays: '', visitingHours: '', googleMapsUrl: '' }, availability: '', availabilityNote: '' })
write('content/doctor/en/reviews.md', { featuredQuote: '', featuredAuthor: '', reviews: [] }, '\nOnly add authentic feedback that you have permission to publish.\n')
copy('docs/new-doctor.md', 'README.md')
copy('content/README.md')
copy('content/doctor/README.md')
copy('content/pages/README.md')
copy('content/settings/README.md')
